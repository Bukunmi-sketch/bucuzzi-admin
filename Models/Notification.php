...
class Notification
{
  ...
  public function unreadNotifyFeedbacks()
  {
    $query = "SELECT * FROM feedbacks WHERE  notify_status=:status ";
    $status = "unread";
    $prepnotify = $this->db->prepare($query);
    $prepnotify->bindParam(':status', $status);
    $prepnotify->execute();
  ...
  /*
        ...
        public function readNotificationfeedbacks(){
              $query="UPDATE Orders SET notify_status='read' WHERE notify_status=:$status ";
               $status="unread";
              $stmt=$this->db->prepare($query);
              $stmt->bindParam(':status',$status );
  ...
  public function countPendingVerifications()
  {
    $status = 'pending';
       ...
       $sql = "SELECT * FROM anonyusers WHERE verification_blue=:status ";
