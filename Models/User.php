...
class User{
  ...
  public function EditCheckusername($username,$sessionid){
    try{
      ...
      $sql="SELECT * FROM anonyusers WHERE username =:username AND id!=:sessionid ";
      $stmt= $this->db->prepare($sql);
  ...
  public function EditCheckEmail($email,$sessionid){
    try{
      ...
      $sql="SELECT * FROM anonyusers WHERE email =:email AND id !=:sessionid ";
      $stmt= $this->db->prepare($sql);
  ...
  public function getUsersPost($userid){
    $querypost="SELECT * FROM posts INNER JOIN anonyusers ON anonyusers.id=posts.user_id WHERE user_id=:sessionid ORDER BY post_id DESC LIMIT 5"; 
  ...
  public function updateProfile($dp,$firstname,$lastname,$username,$email,$biotext,$interest, $country, $website, $sessionid){
    ...
    try{
        ...
        $dpsql="UPDATE anonyusers SET profileimage=:image, firstname=:firstname, lastname=:lastname, username=:username, email=:email,Bio=:description, Country=:country, interest=:interest, website=:website WHERE id=:userid";
  ...
  public function changepassword($sessionid,$newpassword)
  {
    ...
    $query="UPDATE anonyusers SET Password=:newpassword WHERE id=$sessionid ";
  ...
  public function deleteAccount($userid){
    $sqldelete='DELETE FROM anonyusers WHERE id=:userid ';
